$(document).ready(function () {
    ...
    $.ajax({
        url: '../server/get_accounts_admin.php',
        dataType: 'json',
        success: function (data) {
            // Populate DataTable with fetched data
            $('#accounts_admin').DataTable({
                data: data,
                columns: [
            ...
            $('#accounts_admin').on('click', '.edit-btn', function () {
                var data = $('#accounts_admin').DataTable().row($(this).parents('tr')).data();
                // Trigger edit action
                $('#edit_id').val(data.id);
            ...
            $('#accounts_admin').on('click', '.change-pass-btn', function () {
                var data = $('#accounts_admin').DataTable().row($(this).parents('tr')).data();
            ...
            $('#accounts_admin').on('click', '.delete-btn', function () {
                var data = $('#accounts_admin').DataTable().row($(this).parents('tr')).data();
